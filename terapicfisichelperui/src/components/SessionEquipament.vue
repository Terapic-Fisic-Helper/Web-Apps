  ...
  >
    <template v-slot:top>
      <v-toolbar flat color="white">
        ...
        <v-spacer></v-spacer>
        <v-text-field
          label="Search Equipament-Session"
          append-icon="search"
...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      let me = this;
      if (
        confirm("¿Estás seguro que quieres eliminar este EquipamentSessions?")
      )
        axios
          .delete("api/EquipamentSessions/" + item.id, {
            id: item.id,
          })
          .then(function(response) {
            console.log(item.id);
