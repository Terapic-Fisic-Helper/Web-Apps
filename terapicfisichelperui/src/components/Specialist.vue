    ...
    class="elevation-1" style="width:800px">
        <template v-slot:top>
            <v-toolbar flat color="white">
                ...
                <v-spacer></v-spacer>
                <v-text-field label="Search Specialist" append-icon="search" 
                class="text-xs-center" v-model="search" single-line hide-details></v-text-field>
                <v-spacer></v-spacer>
                <v-dialog v-model="dialog" max-width="500px">
                    <template v-slot:activator="{ on }">
                        <v-btn color="primary" dark class="mb-2" v-on="on">New Specialist</v-btn>
                    </template>
                    <v-card>
                        <v-card-title>
                        ...
                        <v-card-text>
                            <v-container>
                                <v-row>
                                    ...
                                    <v-col cols="12" sm="12" md="12">
                                        <v-text-field v-model="specialty" label="Specialty"></v-text-field>
                                    </v-col>
...
<script>
    ...
    export default {
        ...
        methods: {
            listSpecialists() {
                let me= this;
                axios.get('api/Specialists').then(function(response){
            ...
            deleteItem(item) {
                ...
                if(confirm('¿Estás seguro que quieres eliminar este Specialist?'))
                    axios.delete('api/Specialists/' + item.id, {
                        'id': item.id
            ...
            save() {
                ...
                if(this.editedIndex > -1) {
                    axios.put('api/Specialists/' + me.id,{
                        'id': me.id,
                        'userId': me.userId,
            ...
            isValidName() {
                ...
                if(this.specialty.length < 3 || this.specialty.length > 50) {
                    this.validMessage.push("La Descripción debe tener mas de 3 caracteres y menos de 50 caracteres");
                }
