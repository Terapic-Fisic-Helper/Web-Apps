    ...
    class="elevation-1" style="width:800px">
        <template v-slot:top>
            <v-toolbar flat color="white">
                ...
                <v-spacer></v-spacer>
                <v-text-field label="Search Session" append-icon="search" 
                class="text-xs-center" v-model="search" single-line hide-details></v-text-field>
                <v-spacer></v-spacer>
                <v-dialog v-model="dialog" max-width="500px">
                    <template v-slot:activator="{ on }">
                        <v-btn color="primary" dark class="mb-2" v-on="on">New Session</v-btn>
                    </template>
                    <v-card>
                        <v-card-title>
                            ...
                            <v-container>
                                <v-row>
                                    <v-col cols="12" sm="12" md="12">
                                        <v-text-field v-model="specialistId" label="SpecialistId"></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="12" md="12">
                                    ...
                                    <v-col cols="12" sm="12" md="12">
                                        <v-text-field type="datetime" v-model="startHour" label="StartHour"></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="12" md="12">
                                        <v-text-field type="datetime" v-model="endHour" label="EndHour"></v-text-field>
                                    </v-col>
        ...
        methods: {
            ...
            deleteItem(item) {
                ...
                if(confirm('¿Estás seguro que quieres eliminar este Session?'))
                    axios.delete('api/Sessions/' + item.id, {
                        'id': item.id
            ...
            isValidName() {
                ...
                if(this.title.length < 3 || this.title.length > 50) {
                    this.validMessage.push("El titulo debe tener mas de 3 caracteres y menos de 50 caracteres");
                }
